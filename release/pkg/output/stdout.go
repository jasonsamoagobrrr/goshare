...
func NewStdoutput(conf *goshare.Config) *Stdoutput {
	...
	outp.CurrentResults = make([]goshare.Result, 0)
	return &outp
}

func (s *Stdoutput) Banner() {
	version := strings.ReplaceAll(goshare.Version(), "<3", fmt.Sprintf("%s<3%s", ANSI_RED, ANSI_CLEAR))
...
func (s *Stdoutput) Reset() {
	s.CurrentResults = make([]goshare.Result, 0)
}
...
func (s *Stdoutput) GetCurrentResults() []goshare.Result {
	return s.CurrentResults
}

// SetResults sets the result slice
func (s *Stdoutput) SetCurrentResults(results []goshare.Result) {
	s.CurrentResults = results
}

func (s *Stdoutput) Progress(status goshare.Progress) {
	if s.config.Quiet {
...
func (s *Stdoutput) Result(resp goshare.Response) {
...
func (s *Stdoutput) writeResultToFile(resp goshare.Response) string {
...
func (s *Stdoutput) PrintResult(res goshare.Result) {
	switch {
	case s.config.Json:
		s.resultJson(res)
	case s.config.Quiet:
...
func (s *Stdoutput) prepareInputsOneLine(res goshare.Result) string {
	inputs := ""
	if len(res.Input) > 1 {
...
func (s *Stdoutput) resultQuiet(res goshare.Result) {
	fmt.Println(s.prepareInputsOneLine(res))
}

func (s *Stdoutput) resultMultiline(res goshare.Result) {
...
func (s *Stdoutput) resultNormal(res goshare.Result) {
...
func (s *Stdoutput) resultJson(res goshare.Result) {
